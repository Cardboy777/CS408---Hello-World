...
class ShowMessages extends Component {
	...
	componentDidMount() {
		...
		setTimeout(function() {
			...
			if (userData.email === undefined || userData.uid === undefined) {
				return
			}
			var newData = {}
	...
	sendMessage() {
		...
		if (messageBox.value.length < 1) {
			window.alert('You must type a message in the box.')
			return
		}
		if (messageBox.value.split('\n').length > 12) {
			window.alert('You cannot have more than 12 lines of text per message.')
			return
		}
		...
		const fixedMessages = newMessages.map((obj) => {
					...
					<div>
						<div id="chat-area">
							{this.state.messages.length === 0 ? (
								<h1 id="no-msg-msg">
									You haven't Sent Any Messages! Try saying 'Hi'
								</h1>
							) : (
								this.state.messages.map((i) =>
									this.state.messages.length === i + 1 ? (
										<DisplayMessage
											key={i.timestamp}
											me={this.props.uData.uid}
											them={this.props.user.data.uid}
											msg={i}
											last={true}
										/>
									) : (
						...
						<form>
							...
							/>
							<button id="sendMessage" onClick={this.sendMessageButtonEvent}>
								Send
							</button>
						</form>
					</div>
