...
import { myFirestore, myStorage } from './firebase.js';
import './css/chatBoard.css'
import hand_wave from './img/hand-wave.png'
...
export default class ChatBoard extends Component {
  ...
  getListHistory = () => {
    ...
    console.log("in getlistuser");
    this.listMessage.length = 0
    this.setState({ isLoading: true })
    console.log(this.currentPeerUser);
    if (
      this.hashString(this.currentUserId.uid) <=
      this.hashString(this.currentPeerUser)
    ) {
      this.groupChatId = `${this.currentUserId.uid}-${this.currentPeerUser}`
    } else {
      this.groupChatId = `${this.currentPeerUser}-${this.currentUserId.uid}`
    }
  ...
  renderListMessage = () => {
    if (this.listMessage.length > 0) {
      ...
      this.listMessage.forEach((item, index) => {
        if (item.idFrom === this.currentUserId.uid) {
      ...
      return (
        <div className="viewWrapSayHi">
          <span className="textSayHi">Say hi to your new match!</span>
          <img
